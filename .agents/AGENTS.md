...
10. **100% Internationalization & Zero-Hardcoded-String Audit**: Verify that ZERO UI strings, modal titles, form placeholders, button labels, toast notifications, recommendation titles/reasons, or tour tooltips are hardcoded in English. EVERY user-visible string MUST pass through `t(key, language)` or a localized helper. Before claiming any feature or release is ready for production, agents MUST perform an empirical grep/AST search for unlocalized raw text nodes and verify full localization across all supported languages (`en`, `de`, `fr`, `ja`). Never declare production readiness without empirical multi-language test verification.
11. **Holistic Multi-Doc Roadmap Alignment Audit**: Whenever generating next steps, TODOs, or architectural recommendations, agents MUST evaluate the complete corpus of project planning, operational, audit, and memory documents (`docs/DEFERRED.md`, `docs/OPERATIONS.md`, `memory/STATE.md`, `pre_production_multidisciplinary_roundtable_audit.md`, `deep_penetration_testing_report.md`, and `implementation_plan.md`) to synthesize a cohesive, prioritized backlog.
